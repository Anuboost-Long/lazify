...
describe("WorkspacePage synced project controls", () => {
	it("opens a project card at its encoded workspace route", async () => {
		...
		expect(navigate).toHaveBeenCalledWith("/workspace/project/%2Fworkspace%2Fdemo%20project");
	});

	it("opens a focused project card with the Enter key", async () => {
		const user = userEvent.setup();
		renderPage({ projects: [project()] });

		screen.getByRole("button", { name: "Open Demo Project" }).focus();
