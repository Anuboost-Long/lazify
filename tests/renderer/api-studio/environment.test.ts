...
describe("naming what an environment holds", () => {
  ...
  it("carries the value across with the new name", async () => {
    ...
    await waitFor(() =>
      expect(savedSet().environments[0].values).toEqual({ apiRoot: "http://localhost:5000" })
    );
  });
  ...
  it("will not take a name another variable already answers to", async () => {
    ...
    await userEvent.click(within(modal).getByRole("button", { name: "bearerToken" }));
    ...
    expect(within(modal).getByRole("button", { name: "bearerToken" })).toBeTruthy();
  ...
  it("duplicates a row with its value, ready to be renamed", async () => {
    ...
    expect(savedSet().environments[0].values).toEqual({ clientKey: "k-1", clientKey2: "k-1" });
  });
  ...
  it("turns a value into a secret from the row's own menu", async () => {
    ...
    await userEvent.click(
      await screen.findByRole("menuitem", { name: /api_studio\.keep_secret/i })
    );
  ...
  it("keeps a secret hidden until it is asked for", async () => {
    readApiEnvironments.mockResolvedValue({
      ...
      names: {},
      variables: [],
      environments: [{ id: "local", name: "Local", values: { bearerToken: "s3cret" } }]
    });
    readProjectRoutes.mockResolvedValue(scanResult({ routes: [SECURED] }));
    renderPage();
