...
describe("logger", () => {
	...
	it("stops writing to a console that has broken, but keeps the file", async () => {
		const { logError, silenceConsole, getLogFilePath } = await loadLogger();

		silenceConsole();
		logError("main", "after the terminal closed");

		expect(console.error).not.toHaveBeenCalled();
		expect(fs.readFileSync(getLogFilePath(), "utf8")).toContain("after the terminal closed");
	});

	it("silences the console when the stream reports its own error", async () => {
		const { ignoreBrokenConsolePipe, silenceConsole, logError } = await loadLogger();

		ignoreBrokenConsolePipe();
		process.stdout.emit("error", Object.assign(new Error("write EPIPE"), { code: "EPIPE" }));
		process.stdout.removeListener("error", silenceConsole);
		process.stderr.removeListener("error", silenceConsole);

		logError("main", "after the pipe broke");
