...
describe("ASP.NET controller discovery", () => {
	...
	it("keeps the doc summary, bound parameters, body, and declared responses", async () => {
		const result = await scanDotnetProject({ "Controllers/UsersController.cs": CONTROLLER });
		const list = result.routes.find(
			(route) => route.method === "GET" && route.path === "/api/Users",
		)!;
...
`,
	...
	it("asks the environment for one value per scheme a route declares", async () => {
		...
		expect(deriveEnvironmentVariables(result.routes).map((variable) => variable.name)).toEqual(
			expect.arrayContaining(["baseUrl", "X-Client-Key", "Authorization", "X-API-Key"]),
		);
	});
	...
	it("asks the environment for that key, so a value can be filled in", async () => {
		...
		expect(deriveEnvironmentVariables(result.routes).map((variable) => variable.name)).toEqual(
			expect.arrayContaining(["baseUrl", "X-Client-Key"]),
		);
	});
...
describe("ASP.NET minimal API discovery", () => {
	...
	it("takes the servers from launchSettings and the summary from the endpoint", async () => {
		...
		const list = result.routes.find(
			(route) => route.method === "GET" && route.path === "/api/todos",
		)!;
...
describe("what an action says it returns", () => {
	it("reads the model out of the return type when nothing is declared", async () => {
		...
		const byId = result.routes.find(
			(route) => route.path === "/api/SysUser/{id}" && route.method === "GET",
		)!;
	...
	it("returns a list as a list", async () => {
		const result = await scanDotnetProject({ "Controllers/SysUserController.cs": RETURN_TYPES });
		const list = result.routes.find(
			(route) => route.path === "/api/SysUser" && route.method === "GET",
		)!;
...
/**
 * A guard names its header in one file and reads it in another, and a project
 * has plenty of other middleware reading plenty of other headers. The option
 * class that declares the name has to win over an incidental read, whichever
 * file the scan happens to reach first.
 */
describe("finding the header an api key guard actually checks", () => {
	const OPTIONS = `public sealed class ApiKeyOptions
{
    public const string SectionName = "ApiKey";
    public string HeaderName { get; init; } = "X-API-Key";
}`;

	const GUARD = `public class ApiKeyMiddleware(RequestDelegate next, IOptions<ApiKeyOptions> options)
{
	...
	const LOGGING = `public class RequestResponseLoggingMiddleware
{
    // Runs beside the ApiKey middleware and reads a header of its own.
    public async Task InvokeAsync(HttpContext context)
    {
	...
	const STARTUP = `var app = builder.Build();
app.UseMiddleware<ApiKeyMiddleware>();
app.MapControllers();`;

	it("takes the name the options class declares, not a header another middleware reads", async () => {
		const result = await scanDotnetProject({
			"Controllers/UsersController.cs": CONTROLLER,
			"Utility/ApiKeyOptions.cs": OPTIONS,
			"Utility/ApiKeyMiddleware.cs": GUARD,
			"Utility/RequestResponseLoggingMiddleware.cs": LOGGING,
			"Program.cs": STARTUP,
		});

		const headers = result.routes.flatMap((route) =>
			route.security
				.filter((scheme) => scheme.kind === "apiKey")
				.map((scheme) => scheme.parameterName),
		);
