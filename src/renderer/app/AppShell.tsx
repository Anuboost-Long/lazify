...
import { useSetResolvedTheme, useTheme } from "@renderer/shared/hooks/use-theme";
...
export function AppShell() {
	...
	const pages = useMemo(
		() => appSidebarPages.filter((page) => !page.macOnly || globalThis.lazify.platform === "darwin"),
		[],
	);
	...
	const activePageId: AppPageId | null =
		pages.find(
			(page) =>
				...
				(page.id === "templates" && location.pathname.startsWith(`${appRoute.templates}/`)) ||
	...
	const activePage = location.pathname.startsWith(appRoute.initProject)
		? {
	...
	const fullBleed =
		...
		// The home desktop places its windows against the pane itself, so it needs
		// the whole height and no max-width to place them in.
          ...
          unbreakable string pushes the whole window wider than the screen. */}
			<div className="flex min-h-0 min-w-0 flex-1">
              ...
              room, and stays as the hover title when there is not. */}
					<div
						className={clsx("flex h-10 shrink-0 items-center gap-2", "border-b border-border bg-bg px-4")}
					>
                ...
                stays a breadcrumb rather than growing a permanent control. */}
						...
						<UiIcon name={activePage.icon} className="h-3.5 w-3.5 shrink-0 text-accent" />
						...
						<div ref={setCrumbSlot} className="peer flex min-w-0 items-center gap-2" />
                ...
                breadcrumb tail is more specific than a static blurb. */}
						...
						<div ref={setActionSlot} className="ml-auto flex shrink-0 items-center gap-2" />
					</div>
					...
					<PageChromeContext.Provider value={chromeSlots}>
						...
						>
							<div
								className={clsx(
									"mx-auto w-full",
									fullBleed ? "h-full max-w-none p-4" : "max-w-[1560px] px-6 py-6 lg:px-8",
								)}
							>
