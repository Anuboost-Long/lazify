      ...
      {settingsNavItems.map((item) => (
        <button
          ...
          className={clsx(
            ...
            activeSection === item.id ? "bg-accentSoft text-accent" : "text-muted hover:bg-soft hover:text-text"
          )}
        >
          <UiIcon
            name={item.icon}
            className={clsx("h-5 w-5 flex-shrink-0", activeSection === item.id ? "text-accent" : "text-muted")}
          />
          <div>
            <BodyText className="font-semibold leading-none" tone="inherit">
              {t(item.label)}
            </BodyText>
