...
/** The copy itself is the identity: these documents are static and never reorder. */
		...
		<div className="mx-auto grid w-full max-w-5xl gap-8 lg:grid-cols-[minmax(0,1fr)_13rem]">
			<article className="min-w-0">
				...
				<header className="rounded-[30px] border border-border bg-soft p-8 shadow-panel">
					...
					<BodyText className="mt-5 max-w-2xl text-[15px] leading-7">{doc.summary}</BodyText>
				</header>
				...
				<div className="mt-6 rounded-[30px] border border-border bg-soft p-8 shadow-panel">
					{sections.map((section, index) => (
						<section
							key={section.id}
							id={section.id}
							className={clsx("scroll-mt-4", index > 0 && "mt-8 border-t border-border pt-8")}
						>
							<SectionTitle>{section.heading}</SectionTitle>
							<div className="mt-3 flex flex-col gap-3">
								{section.blocks.map((block) =>
									block.kind === "paragraph" ? (
										<BodyText key={blockKey(block)} tone="muted" className="leading-7">
											{block.text}
										</BodyText>
									) : (
										<ul
										...
										>
											{block.items.map((item) => (
												<li key={item}>
													<BodyText as="span" tone="muted" className="leading-7">
														{item}
													</BodyText>
												</li>
											))}
