...
import { projectFolderName } from "./project-name";
	...
	return (
		<aside className={railPanelShell(variant)}>
			...
			<DeleteTaskConfirm
				...
				onConfirm={(task) => {
					setDeleting(null);
					setEditor((current) => (current.task?.id === task.id ? { open: false, task: null } : current));
					void remove(task.id);
				}}
			...
			<TaskDetailModal
				...
				projectName={projectFolderName(projectPath)}
				presets={presets}
				onSetStatus={(status) => {
					if (!opened) return;
