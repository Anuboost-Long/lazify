...
export function EnvironmentPage({ report, loading, onRefresh }: Readonly<EnvironmentPageProps>) {
	...
	const [updateInfo, setUpdateInfo] = useState<
		import("@renderer/shared/types/lazify").ToolUpdateInfo | null
	>(null);
	...
	const [uninstallTarget, setUninstallTarget] = useState<ToolScanReport["tools"][number] | null>(
		null,
	);
          ...
          only thing worth putting up there is the action. */}
			<PageActions>
				...
				>
					<UiIcon
						name="refresh-circle"
						className={clsx("h-3.5 w-3.5 text-muted group-hover:text-accent", loading && "animate-spin")}
					/>
          ...
          install to install, so it scans on its own. */}
				...
				<InstallToolModal
					...
					onInstalled={(toolName) => {
						closeInstall();
						void refreshSingleTool(toolName);
					}}
				/>
