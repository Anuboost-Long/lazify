...
export function UpdateToolModal({
	tool,
	updateInfo,
	open,
	onClose,
	onUpdated,
}: Readonly<UpdateToolModalProps>) {
					...
					<div className="flex items-center justify-between gap-4">
						<div>
							<OverlineText className="text-muted">
								{hasUpdate
									? t(translation.UpdateToolModal.UpdateAvailable)
									: t(translation.UpdateToolModal.UpToDate)}
							</OverlineText>
					...
					{state === "result" && !hasUpdate && (
						<div className="flex items-start gap-3 rounded-[20px] border border-success/20 bg-success/5 px-4 py-4">
							...
							<div className="min-w-0">
								<BodyText className="font-semibold text-success">
									{t(translation.UpdateToolModal.AlreadyLatest)}
								</BodyText>
					...
					{state === "result" && hasUpdate && tool && (
						<div className="space-y-4">
								...
								<div className="flex items-center gap-3">
									...
									<div className="flex min-w-0 flex-1 items-center gap-3">
										<div className="min-w-0 flex-1">
											<OverlineText className="text-[10px] text-muted">
												{t(translation.UpdateToolModal.Installed)}
											</OverlineText>
											<MonoText
												as="p"
												title={currentVersion ?? undefined}
												className="mt-1 truncate text-sm font-semibold text-text"
											>
										...
										<div className="min-w-0 flex-1">
											<OverlineText className="text-[10px] text-accent">
												{t(translation.UpdateToolModal.Latest)}
											</OverlineText>
							...
							<div className="rounded-[16px] border border-border bg-bg px-4 py-3">
								...
								<MonoText as="code" className="break-all text-sm text-accent">
									{tool.updateCommand}
								</MonoText>
							</div>
						</div>
						...
						<div className="flex flex-col items-center gap-4 py-6">
							...
							<div className="text-center">
								<BodyText className="font-semibold text-text">
									{t(translation.UpdateToolModal.Updating, { name: tool?.displayName })}
								</BodyText>
								<BodyText className="mt-1 text-xs text-muted">
									{t(translation.UpdateToolModal.UpdatingDesc)}
								</BodyText>
							</div>
						</div>
					)}
								...
								<div className="flex items-start gap-3">
									...
									<div>
										<BodyText className={clsx("font-semibold", success ? "text-success" : "text-error")}>
											{success
												? t(translation.UpdateToolModal.UpdateSuccess, { name: tool?.displayName })
												: t(translation.UpdateToolModal.UpdateFailed)}
										</BodyText>
										<BodyText className="mt-1 text-xs text-muted">
											{success
												? t(translation.UpdateToolModal.SuccessDesc)
												: t(translation.UpdateToolModal.FailedDesc)}
										</BodyText>
									</div>
								</div>
							</div>
