...
/** One card, and which of install / check / versions it offers. */
function CategoryTool({
	...
	const action = () => {
		if (isNodeEntry) return onNvmAction;
		if (canUpdate) return () => onUpdate(tool);
		if (canInstall) return () => onInstall(tool);

		return undefined;
	};

	const actionLabel = () => {
		if (isNodeEntry) return t(translation.GlobalTerm.Versions);
		if (canUpdate) return t(translation.Environment.Check);
		if (canInstall) return t(translation.GlobalTerm.Install);
