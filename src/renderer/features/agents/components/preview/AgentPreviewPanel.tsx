		...
		>
			...
			>
				...
				<Control
					icon="media-video"
					label={t(
						pagePip.open ? translation.Agents.PreviewPipPageClose : translation.Agents.PreviewPipPage,
					)}
					active={pagePip.open}
					disabled={!target}
					onClick={() => {
				...
				<Control
					...
					disabled={!target || !ready}
					onClick={() =>
						drive((view) => (view.isDevToolsOpened() ? view.closeDevTools() : view.openDevTools()))
					}
				/>
			...
			{pipNotice ? (
				<SmallText className="!text-muted border-b border-border px-3 py-1.5">{pipNotice}</SmallText>
			) : null}
			...
			<div className="relative min-h-0 flex-1">
				{target ? (
					<webview
						...
						partition={PREVIEW_PARTITION} // NOSONAR: Electron's <webview>, typed in vite-env.d.ts
