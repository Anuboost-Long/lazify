...
import { MONITOR_PANEL_SIZES, type MonitorPanelSize } from "../hooks/use-monitor-panels";
...
/** The four quadrants of the preview square, in reading order. */
const CELLS = ["top-left", "top-right", "bottom-left", "bottom-right"] as const;
	...
	return (
		...
		>
			{CELLS.map((cell, index) => (
				<span
					key={cell}
					className={clsx("rounded-[2px]", SIZE_CELLS[size][index] ? "bg-accent/70" : "bg-text/10")}
				/>
			))}
		</div>
	);
			...
			>
				<div className="flex items-center justify-between gap-4 border-b border-border px-6 py-5">
					<div className="min-w-0">
						<OverlineText className="truncate text-muted">{panelLabel ?? ""}</OverlineText>
					...
					{MONITOR_PANEL_SIZES.map((size) => {
						...
						return (
							...
							>
								...
								<UiIcon
									name="check-circle"
									className={clsx("h-4 w-4 shrink-0 text-accent", selected ? "opacity-100" : "opacity-0")}
								/>
							</button>
						);
					})}
