...
const STAGE_INDEX: Record<SetupStage, number> = {
	project: 0,
	source: 1,
	script: 2,
};

const STAGE_TITLE: Record<SetupStage, string> = {
	project: translation.Agents.MonitorChooseProject,
	source: translation.Agents.MonitorChooseSource,
	script: translation.Agents.MonitorChooseScript,
};
...
export function AgentMonitorSetupModal({
	...
	const title = t(STAGE_TITLE[stage]);

	const visibleProjects = useMemo(
		() =>
			projects.filter((candidate) => matches(query, candidate.projectName, candidate.projectPath)),
		[projects, query],
	);
