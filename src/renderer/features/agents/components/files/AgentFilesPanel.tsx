...
export function AgentFilesPanel({
	...
	// Under the row rather than under the pointer, so the menu reads as belonging
	// to the file it acts on.
	const openMenu = (event: React.MouseEvent, node: ImportedProjectIndexNode) => {
		const row = event.currentTarget.getBoundingClientRect();

		setMenu({ node, x: row.left, y: row.bottom + 4 });
	};
	...
	return (
		<aside className={railPanelShell(variant)}>
			...
			<AgentFileModal
				...
				focusLine={symbolLine && symbolLine.path === selected?.absolutePath ? symbolLine.line : null}
