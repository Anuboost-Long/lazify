...
export function AgentPickerModal({
	...
	const title = () => {
		if (creating) return t(translation.Agents.CustomAgentTitle);
		if (browsing) return t(translation.Agents.ResumeSession);

		return t(translation.Agents.ChooseAgent);
	};
	...
	const body = () => {
		...
		if (browsing) {
			return <ResumeSessionList sessions={sessions} resumable={resumable} onSelect={handleSelect} />;
		}
					...
					<div className="flex items-center justify-between gap-4">
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<SectionTitle className="mt-1 text-2xl">{title()}</SectionTitle>
							</div>
						</div>
						<button
							type="button"
