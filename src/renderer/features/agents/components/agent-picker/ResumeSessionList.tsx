...
export function ResumeSessionList({
	...
	const rows = () => {
		if (sessions === null) {
			return <CaptionText tone="muted">{t(translation.GlobalTerm.Loading)}</CaptionText>;
		}

		if (resumable.length === 0) {
			return <CaptionText tone="muted">{t(translation.Agents.ResumeSessionEmpty)}</CaptionText>;
		}
