...
import { FormatChangesModal } from "./FormatChangesModal";
...
export function AgentChangesPanel({
	...
	const { busy, outcome, configFile } = useProjectFormatter(projectPath);

	useEffect(() => setSelected(null), [projectPath]);

	const formatterNote = () => {
		if (busy) return t(translation.Agents.FormatRunning);

		if (outcome) {
			if (outcome.failed.length > 0) {
				return t(translation.Agents.FormatFailed, { count: outcome.failed.length });
			}

			return outcome.formatted.length === 0
				? t(translation.Agents.FormatNone)
				: t(translation.Agents.FormatDone, { count: outcome.formatted.length });
		}

		return configFile
			? t(translation.Agents.FormatByProject, { config: configFile })
	...
	return (
		<aside className={railPanelShell(variant)}>
			...
			<div className="min-h-0 flex-1 overflow-auto p-1.5">
				{changes.length === 0 ? (
					<SmallText className="!text-muted px-1.5 py-2">{t(translation.Agents.NoChanges)}</SmallText>
				) : (
					changes.map((change) => {
			...
			<footer className="flex items-center gap-2 border-t border-border px-2.5 py-2">
				<SmallText
					as="span"
					className={clsx(
						"min-w-0 flex-1 truncate",
						outcome && outcome.failed.length > 0 ? "!text-error" : "!text-muted",
					)}
					title={formatterNote()}
				>
					{formatterNote()}
				...
				<button
					...
					className={clsx(
						"flex h-7 shrink-0 items-center gap-1.5 rounded-lg px-2.5",
				...
				>
					<UiIcon name="sparks" className={clsx("h-3 w-3", busy && "animate-pulse")} />
					{t(translation.Agents.FormatChanges)}
				</button>
			</footer>
