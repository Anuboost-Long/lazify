...
export function useMonitorPanels() {
  ...
  const clearAll = useCallback(async () => {
    ...
    await Promise.all(
      running.map((panel) =>
        globalThis.lazify.stopScript(panel.runId).catch(() => {
        }),
      ),
    );
  }, [commitLayout, setPanels]);
  ...
  useEffect(() => {
    return globalThis.lazify.onScriptStatus((event) => {
      ...
      setPanels((current) =>
        current.map((panel) =>
          panel.runId === event.runId ? { ...panel, exited: true } : panel,
        ),
      );
    });
  }, [setPanels]);
  ...
  const target =
    panels.find((panel) => panel.runId === layout.activeRunId) ?? panels[0] ?? null;
