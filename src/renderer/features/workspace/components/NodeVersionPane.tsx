...
/** The rail down the left of a row: running, pinned, or neither. */
function railTone(current: boolean, pinned: boolean): string {
	if (current) return "bg-success/70";
	if (pinned) return "bg-accent";

	return "bg-border/30 group-hover:bg-border/60";
}

function chipTone(current: boolean, pinned: boolean): string {
	if (current) return "bg-success/10 text-success";
	if (pinned) return "bg-accent/10 text-accent";

	return "bg-bg text-muted";
}

export function NodeVersionPane({
	projectPath: _projectPath,
	pinnedVersion,
	onVersionChange,
}: Readonly<NodeVersionPaneProps>) {
	...
	const handleSwitch = async (version: string) => {
		...
		try {
			...
			if (result.success) {
				...
				setFeedback({
					ok: false,
					message: result.output || t(translation.NodeVersionPane.SwitchError),
				});
			}
		} catch (err) {
			setFeedback({
				ok: false,
				message: err instanceof Error ? err.message : t(translation.NodeVersionPane.SwitchError),
			});
		} finally {
			setSwitching(false);
		}
				...
				{!loading && versionList && !nvmAvailable && (
					<div className="flex flex-col items-center justify-center rounded-[20px] border border-dashed border-border bg-soft/30 py-10 text-center">
						...
						<CardTitle className="mt-4 text-base">
							{t(translation.NodeVersionPane.NvmUnavailable)}
						</CardTitle>
									...
									>
										...
										<div className="flex items-center gap-3 py-2.5 pl-5 pr-4">
											...
											<div className="min-w-0 flex-1">
												...
												<div className="mt-0.5 flex items-center gap-1.5">
													{v.current && (
														<PillText
															as="span"
															className="rounded-full border border-success/25 bg-success/8 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.15em] text-success"
														>
													...
													{v.lts && (
														<PillText
															as="span"
															className="rounded-full border border-border bg-bg px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.15em] text-muted"
														>
											...
											<LabelButton
												label={
													isPinned
														? translation.NodeVersionPane.Switch
														: translation.NodeVersionPane.PinAndSwitch
												}
												variant="accent"
												loading={switching}
