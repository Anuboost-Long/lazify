...
/** How far one arrow-key press moves the terminal's top edge. */
const RESIZE_STEP = 24;

export function ScriptsPane({ projectPath }: Readonly<ScriptsPaneProps>) {
	...
	const loadScripts = async () => {
		...
		try {
			...
			setLoadError(err instanceof Error ? err.message : t(translation.ScriptsPane.LoadError));
		} finally {
			setLoading(false);
		}
	};
	...
	const isActiveTabBusy = activeTab?.status === "running" || activeTab?.status === "pending";
		...
		<div>
			...
			<div className="space-y-4 p-4">
					...
					<div className="grid gap-1.5">
						{scriptEntries.map(([name, command]) => (
							<ScriptRow
								...
								name={name}
								command={command}
								isRunningHere={isActiveTabBusy && activeTab?.scriptName === name}
				...
				<div className="overflow-hidden rounded-[20px] border border-black/[0.06] dark:border-white/[0.04]">
					...
					<ScriptTerminalBodies tabs={tabs} activeTabId={activeTabId} height={terminal.height} />

					{/* ── Drag handle ── */}
					{/* A separator rather than a bare div: the drag is a mouse gesture,
					    and the arrow keys are how it is done without one. */}
					<div
						role="separator"
						aria-orientation="horizontal"
						aria-label={t(translation.ScriptsPane.ResizeTerminal)}
						tabIndex={0}
						...
						onKeyDown={(event) => {
							...
							terminal.resizeBy(event.key === "ArrowUp" ? -RESIZE_STEP : RESIZE_STEP);
						}}
						className={clsx(
							"flex h-[11px] cursor-ns-resize select-none items-center justify-center",
							"border-t border-border bg-soft focus-visible:outline-none focus-visible:bg-accent/20",
