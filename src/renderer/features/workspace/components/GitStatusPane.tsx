...
export function GitStatusPane({
	...
	/** The list area: what is loading, what is clean, or what has changed. */
	const workingTree = (repo: ProjectGitStatusResult) => {
		if (loading) {
			return (
		...
		if (!repo.hasUncommittedChanges) {
			return (
				<div
					className={clsx(
						"mt-4 flex flex-1 items-center justify-center p-8 text-center",
						!flush && "rounded-[20px] border border-dashed border-border bg-soft/30",
					)}
				>
					<div>
						<CardTitle className="text-lg">{t(translation.GitStatus.TreeClean)}</CardTitle>
						<BodyText className="mt-3 text-muted">{t(translation.GitStatus.TreeCleanDesc)}</BodyText>
					</div>
				</div>
			);
		}
		...
		return (
			<div className="mt-1 min-h-0 flex-1 space-y-1 overflow-y-auto">
				{(() => {
					const groups = groupEntries(repo.entries);

					const renderEntries = (entries: GitStatusEntry[], group: "staged" | "unstaged") => {
						const shared = {
							entries,
							selectedPath,
							group,
						...
						return viewMode === "list" ? <GitStatusList {...shared} /> : <GitStatusTree {...shared} />;
					};
	...
	return (
		...
		>
			...
			>
				...
				{(busy || loading) && (
					<PillText className="text-accent">{t(translation.GlobalTerm.Checking)}</PillText>
				)}
				{/* Flat list or folder tree, the way source control views let you
            choose once a change set gets deep. */}
				<Tooltip
					content={t(
						viewMode === "list" ? translation.GitStatus.ViewAsTree : translation.GitStatus.ViewAsList,
					)}
					side="top"
				>
					<button
				...
				{loading && !gitStatus && (
					...
					>
						<div>
							...
							<BodyText className="mt-3 text-muted">{t(translation.GitStatus.CheckingDesc)}</BodyText>
						</div>
					</div>
				)}
				...
				{!loading && gitStatus && !gitStatus.isGitRepo && (
					...
					>
						<div className="max-w-sm">
							...
							<BodyText className="mt-3 text-muted">{t(translation.GitStatus.NoRepoDesc)}</BodyText>
						</div>
					</div>
				)}
