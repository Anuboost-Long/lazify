...
/** The card's own tint, matching the action badge inside it. */
function rowTone(action: PackageMatch["action"]): string {
	if (action === "keep") return "border-border bg-soft/30";
	if (action === "downgrade") return "border-warning/20 bg-warning/5";

	return "border-accent/15 bg-accent/5";
}

function ActionBadge({ action }: Readonly<{ action: PackageMatch["action"] }>) {
...
function CompatBadge({ status }: Readonly<{ status: PackageMatch["compatibility"] }>) {
	const { t } = useTranslation();
...
function SummaryPills({ report }: Readonly<{ report: VersionMatchReport }>) {
	const { t } = useTranslation();
	...
	return (
		<div className="flex flex-wrap gap-2">
			...
			{compatible > 0 && (
				<PillText
					as="span"
					className="rounded-full border border-accent/20 bg-accent/8 px-3 py-1 text-accent"
				>
			...
			{needsFix > 0 && (
				<PillText
					as="span"
					className="rounded-full border border-warning/25 bg-warning/10 px-3 py-1 text-warning"
				>
...
export function PackageVersionPane({ projectPath }: Readonly<PackageVersionPaneProps>) {
	const { t } = useTranslation();
				...
				{!report && !scanning && (
					<div className="flex items-center justify-center rounded-[20px] border border-dashed border-border bg-soft/30 py-12 text-center">
						<div>
							...
							<BodyText className="mt-2 text-muted">{t(translation.PackageDoctor.EmptyDesc)}</BodyText>
						</div>
					</div>
				)}
				...
				{report && !scanning && !fixing && (
					<div className="space-y-4">
						...
						<div className="rounded-xl border border-border px-3 py-2 text-xs text-muted">
							{t(translation.PackageDoctor.AnchoredTo, {
								package: report.anchorPackage,
								version: report.anchorVersion,
							})}
						</div>
						...
						{report.packages.length > 0 && (
							<div className="grid gap-2">
								{report.packages.map((pkg) => (
									<div
										key={pkg.name}
										className={clsx("rounded-[18px] border px-4 py-3", rowTone(pkg.action))}
									>
										<div className="flex items-start justify-between gap-3">
											<div className="min-w-0 flex-1">
												<div className="flex items-center gap-2">
													<MonoText className="truncate text-sm font-semibold text-text">{pkg.name}</MonoText>
													<CompatBadge status={pkg.compatibility} />
												</div>
												<div className="mt-1 flex items-center gap-2 text-xs text-muted">
													<Typography as="span" variant="caption" className="text-muted">
														Current:{" "}
														<MonoText as="span" className="text-xs text-text">
															{pkg.currentSpec}
														</MonoText>
													</Typography>
