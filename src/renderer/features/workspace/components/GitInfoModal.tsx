...
/** What the sheet says under its title before anything is known. */
function HeaderSubtitle({
	loading,
	gitStatus,
}: Readonly<{ loading: boolean; gitStatus: ProjectGitStatusResult | null }>) {
	const { t } = useTranslation();

	if (loading) return <>{t(translation.GitStatus.CheckingStatus)}</>;
	if (!gitStatus?.isGitRepo) return <>{t(translation.GitStatus.NoLinkedRepo)}</>;

	return <>{gitStatus.branch ?? t(translation.GitStatus.DetachedHead)}</>;
}

/** The repository itself: where it lives, which branch, and what has changed. */
function RepoDetails({ gitStatus }: Readonly<{ gitStatus: ProjectGitStatusResult }>) {
	const { t } = useTranslation();
	const changeCount = gitStatus.entries.length;
		...
		<>
			...
			<Section icon="code" label={t(translation.GitStatus.WorkingTree)}>
				{!gitStatus.hasUncommittedChanges ? (
					<BodyText className="text-sm text-muted">{t(translation.GitStatus.TreeCleanDesc)}</BodyText>
				) : (
					<>
...
function ModalBody({
	...
	return <RepoDetails gitStatus={gitStatus} />;
}
		...
		<BaseModal open={open} onClose={onClose} contentClassName="p-6 md:p-8">
			<div className="flex max-h-[82vh] w-[min(56rem,calc(100vw-4rem))] flex-col overflow-hidden rounded-[20px] border border-border bg-soft shadow-panel">
				...
				<div className="flex shrink-0 items-center gap-3 border-b border-border px-6 py-4">
					...
					<div className="min-w-0 flex-1">
						<CardTitle className="truncate text-base">{t(translation.GitStatus.Title)}</CardTitle>
						<BodyText className="truncate text-xs text-muted">
							<HeaderSubtitle loading={loading} gitStatus={gitStatus} />
						</BodyText>
					</div>
					<IconButton
						icon="xmark"
				...
				<div className="min-h-0 flex-1 overflow-y-auto bg-bg">
					<ModalBody loading={loading} gitStatus={gitStatus} />
				</div>
			</div>
		</BaseModal>
	);
