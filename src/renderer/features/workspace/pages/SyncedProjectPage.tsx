...
import { SonarScanPane } from "@renderer/features/workspace/components/sonar-scan/SonarScanPane";
...
export function SyncedProjectPage({
	...
	const reveal = revealFilePath
		? {
				filePath: revealFilePath,
				line: Number.isFinite(revealLine) && revealLine > 0 ? revealLine : null,
			}
		: null;
	...
	useEffect(() => {
		...
		const load = async () => {
			try {
				...
				const result = await globalThis.lazify.importProjectIndexFromDirectory(decodedPath);
				if (!cancelled) setProjectData(result);
			} catch (error) {
				if (!cancelled) {
					setProjectData(null);
					setErrorMessage(
						error instanceof Error ? error.message : t(translation.SyncedProject.LoadError),
					);
				}
			} finally {
	...
	const handleInstallDependencies = async () => {
		...
		try {
			const result = await globalThis.lazify.installProjectDependencies(syncedProject.projectPath);
			setInstallFeedback(
				result.success
					? {
							tone: "success",
	...
	const toolViews: SidebarView[] = syncedProject
		? [
				...
				{
					id: "sonar",
					label: t(translation.SonarScan.Title),
					icon: "radar",
					content: <SonarScanPane projectPath={syncedProject.projectPath} />,
				},
				{
					id: "runtime",
				...
				<div className="min-h-0 flex-1">
					<ProjectCodeActions projectPath={syncedProject.projectPath}>
						<SyncedProjectViewer
							...
							reveal={reveal}
							toolViews={toolViews}
							onOpenConsole={() => void globalThis.lazify.openTerminal(syncedProject.projectPath)}
