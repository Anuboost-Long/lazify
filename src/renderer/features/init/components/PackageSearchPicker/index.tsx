...
export function PackageSearchPicker({
  ...
  >({});
  const [templatePackages, setTemplatePackages] = useState<
        ...
        if (!cancelled) {
          ...
          setErrorMessage(
            error instanceof Error
              ? error.message
              : t(translation.PackageSearch.NpmError)
          );
        }
      } finally {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setTemplatePackagesError(
            error instanceof Error
              ? error.message
              : t(translation.PackageSearch.TemplateError)
          );
        }
      } finally {
  ...
  const removePackage = (packageName: string) => {
    onChange(
      selectedPackages.filter((item) => item !== packageName).join(", ")
    );
  };
