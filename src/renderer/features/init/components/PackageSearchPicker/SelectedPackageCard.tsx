...
export function SelectedPackageCard({ pkg, busy, onRemove }: Readonly<SelectedPackageCardProps>) {
			...
			<div className="flex items-start justify-between gap-4">
				<div className="min-w-0">
					<div className="flex flex-wrap items-center gap-2">
						<CardTitle as="span" className="text-text">
							{pkg.name}
						</CardTitle>
						{pkg.version ? (
						...
						{pkg.publisher ? (
							<PillText
								as="span"
								className="rounded-full border border-border bg-soft px-2 py-1 text-[10px] text-muted"
							>
								{pkg.publisher}
							</PillText>
						) : null}
