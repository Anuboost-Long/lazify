...
export function useTemplateTree({
	...
	useEffect(() => {
		...
		setExpandedIds((current) =>
			Array.from(new Set([...current, ...collectFolderIds(resolvedInitialTree)])),
		);
		const firstFileId = findFirstFileId(resolvedInitialTree);
		setSelectedId((current) => current ?? firstFileId);
		setActiveFileId((current) => current ?? firstFileId);
		setOpenFileIds((current) => (current.length > 0 || !firstFileId ? current : [firstFileId]));
	}, [initialTree, replaceTreeOnInitialChange, resolvedInitialTree]);
	...
	const activeTab = openFiles.find((tab) => tab.filePath === activeFileId) ?? null;
	...
	const handleSelectNode = (nodeId: string) => {
		...
		setOpenFileIds((current) => (current.includes(node.id) ? current : [...current, node.id]));
	...
	const handleOpenContextMenu = (event: React.MouseEvent<HTMLButtonElement>, nodeId: string) => {
