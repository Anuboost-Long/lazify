	...
	useEffect(() => {
		const onMessage = (event: MessageEvent) => {
			...
			void navigator.clipboard
				...
				// NOSONAR: the frame is sandboxed without allow-same-origin, so its origin is opaque and "*" is the only target it can be addressed by
				.then(() => frame.current?.contentWindow?.postMessage({ type: "lazify-doc-copied" }, "*"))
				.catch(() => undefined);
		};

		globalThis.addEventListener("message", onMessage); // NOSONAR: the sender is checked against this pane's own frame above, which an opaque origin cannot be
	...
	const exportAs = (format: DocFormat) => {
		...
		void globalThis.lazify
			...
			.then((result) => {
				if (result) {
					setNote({
						text: t(translation.ApiStudio.DocExported, { path: result.filePath }),
						failed: false,
					});
				}
			})
	...
	return (
		<div className="flex min-h-0 flex-1 flex-col">
			<div className="flex flex-wrap items-center gap-1.5 border-b border-border px-4 py-2.5">
				<button type="button" onClick={() => setReloadAt(Date.now())} className={ACTION_CLASS}>
				...
				<button type="button" disabled={busy} onClick={() => exportAs("html")} className={ACTION_CLASS}>
					<UiIcon name="html" className="h-3.5 w-3.5" />
					{t(translation.ApiStudio.DocExportHtml)}
				</button>

				<button type="button" disabled={busy} onClick={() => exportAs("pdf")} className={ACTION_CLASS}>
