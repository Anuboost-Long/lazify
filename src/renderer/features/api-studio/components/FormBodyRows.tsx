...
export function FormBodyRows({ entries, readOnly = false, onChange }: Readonly<FormBodyRowsProps>) {
	...
	return (
		<div className="flex flex-col gap-2">
			{entries.map((entry, index) => (
				// NOSONAR: a form entry is a position in the request contract and carries no id of its own
				<div key={index} className="flex items-center gap-2">
					...
					<button
						...
						onClick={() =>
							entry.kind === "file" ? replace(index, { kind: "text", value: "" }) : chooseFile(index)
						}
						className={clsx(
