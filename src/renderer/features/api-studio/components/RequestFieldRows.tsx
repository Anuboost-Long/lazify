...
function FieldRow({
	...
	const name = () => {
		...
		return (
			<span className="min-w-0 truncate font-mono text-xs font-semibold text-text">{field.label}</span>
		);
	};

	const menuItems = () => [
		...(onRepeat
			? [{ key: "repeat", label: t(translation.ApiStudio.AddAnotherValue), onSelect: onRepeat }]
			: []),
				...
				<tbody className="divide-y divide-border">
					{fields.map((field) => (
						<FieldRow
							...
							onRemove={(field.added || field.repeat) && onRemove ? () => onRemove(field.key) : null}
						/>
					))}
				</tbody>
			</table>
