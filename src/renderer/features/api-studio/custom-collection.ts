...
export type { CustomRequestDraft } from "@main/api-studio/custom-collections";
export type { CustomCollection, CustomFolder, CustomRequest };
...
export function withFolder(
	...
	return {
		...collection,
		folders: collection.folders.map((folder) => (folder.id === folderId ? change(folder) : folder)),
	};
}
...
function movedRequest(
	...
	const carried = held
		? [
				...held.collection.requests,
				...held.collection.folders.flatMap((folder) => folder.requests),
	...
	return withRequests(without, target, (requests) => {
		const at = beforeRequestId ? requests.findIndex((request) => request.id === beforeRequestId) : -1;
...
function reorderedCollections(
	collections: CustomCollection[],
	draggedId: string,
	targetId: string,
): CustomCollection[] {
	const fromIndex = collections.findIndex((collection) => collection.id === draggedId);
	const toIndex = collections.findIndex((collection) => collection.id === targetId);
...
function droppedFolder(
	...
	if (target.kind === "folder") {
		...
		return into ? movedFolder(collections, folderId, into.id, target.id) : collections;
	}

	if (target.kind === "collection") return movedFolder(collections, folderId, target.id, null);
...
function droppedRequest(
	...
	if (target.kind === "folder") {
		...
		return into
			? movedRequest(collections, requestId, { collectionId: into.id, folderId: target.id }, null)
			: collections;
	}

	if (target.kind === "collection") {
		return movedRequest(collections, requestId, { collectionId: target.id, folderId: null }, null);
	}
...
export function movedNode(
	...
	if (dragged.kind === "collection" && target.kind === "collection") {
		return reorderedCollections(collections, dragged.id, target.id);
	}

	if (dragged.kind === "folder") return droppedFolder(collections, dragged.id, target);
	if (dragged.kind === "request") return droppedRequest(collections, dragged.id, target);
