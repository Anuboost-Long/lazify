...
async function detailsFor(projectPath: string, routes: SavedRoute[]): Promise<SavedRoute[]> {
	...
	const read = await Promise.all(
		folders.map((folder) => globalThis.lazify.readRouteDetails(projectPath, folder).catch(() => [])),
	);
...
function withoutExample(
	...
	return requests.map((request) =>
		request.id === requestId
			? { ...request, examples: request.examples.filter((example) => example.id !== exampleId) }
			: request,
	);
}
...
export function useCustomCollection(projectPath: string) {
	...
	const apply = (change: (collections: CustomCollection[]) => CustomCollection[]) => {
		...
		if (projectPath) {
			void globalThis.lazify
				.saveApiCollections(projectPath, next)
				.catch(() =>
	...
	return {
		...
		removeExample: (requestId: string, exampleId: string) =>
			apply((currentCollections) =>
				mapRequests(currentCollections, (requests) => withoutExample(requests, requestId, exampleId)),
			),
		...
		saveDraft: (requestId: string, draft: CustomRequestDraft | null, examples?: SavedExample[]) => {
