...
function formMediaTypeOf(declared: string | undefined, entries: FormEntry[]): string {
	if (entries.some((entry) => entry.kind === "file")) return MULTIPART_MEDIA_TYPE;
	if (declared && isFormMediaType(declared)) return declared;

	return URLENCODED_MEDIA_TYPE;
}

/** A base URL joins a path that brings its own leading slash. */
...
export function useRequestDraft(
	...
	const formMediaType = formMediaTypeOf(variant?.mediaType, entries);
	...
	const baseUrl = withoutTrailingSlashes(
		resolveVariable(variables, values, BASE_URL_VARIABLE) ?? "",
	);

	const exampleRequest = (target: ApiRequestDraft): ExampleRequest | null =>
		route ? { ...target, route, baseUrl, fields, mode, json, entries, scripts } : null;
	...
	const run = async (target: ApiRequestDraft) => {
		...
		try {
			...
			store(
				result.outcome.ok ? { ...result.outcome.response, receivedAt: new Date().toISOString() } : null,
			);
		} catch (error) {
			setScriptRuns({ pre: null, post: null });
			setOutcome({
	...
	const shown: (SavedResponse | ApiResponseSummary) | null = outcome?.ok ? outcome.response : null;
	...
	return {
		...
		body: {
			...
			reset: () => {
				/** The body goes back to what the route declares; the scripts are the
            user's own work, so they are put back after the seeding clears them. */
				const written = scripts;
		...
		scriptRuns,
		setField: (key: string, value: string) => setFields((current) => ({ ...current, [key]: value })),
