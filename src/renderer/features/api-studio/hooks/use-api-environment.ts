...
import type { ApiEnvironment, ApiEnvironmentSet, ApiVariable, SavedRouteSummary } from "../types";
...
export function useApiEnvironment(projectPath: string, routes: SavedRouteSummary[]) {
	...
	const save = (next: ApiEnvironmentSet) => {
		...
		const known = new Set(declared.map((variable) => variable.key));
		const secretNames = [
			...declared.filter((variable) => variable.secret).map((variable) => variable.key),
			...next.environments
	...
	const duplicateVariable = (key: string) => {
		...
		const value = active.values[copied.key];
		...
		save({
			...
			environments: set.environments.map((environment) =>
				environment.id === active.id && value !== undefined
					? { ...environment, values: { ...environment.values, [variable.key]: value } }
					: environment,
			),
		});
	...
	const removeVariable = (key: string) => {
		...
		save({
			...
			environments: set.environments.map((environment) => {
				const { [key]: dropped, ...values } = environment.values;
	...
	const renameVariable = (key: string, rename: string) => {
		...
		save({
			...
			variables: declared
				? set.variables.map((variable) => (variable.key === key ? { ...variable, name } : variable))
				: set.variables,
		});
	};

	const missing = variables.filter(
		(variable) => !active.values[variable.key]?.trim() && !variable.defaultValue,
	);
