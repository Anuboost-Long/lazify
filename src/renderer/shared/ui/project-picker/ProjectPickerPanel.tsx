...
export function ProjectPickerPanel({
	...
	return (
		<div className={clsx("flex min-h-0 flex-1 flex-col", className)}>
			<div
				className={clsx(
					"scroll-visible grid min-h-0 flex-1 auto-rows-max grid-cols-1 content-start gap-2 overflow-y-auto py-2",
					"sm:grid-cols-2",
					inset && "px-6",
				)}
			>
			...
			<footer className={clsx("shrink-0 border-t border-border py-3", inset && "px-6")}>
				<ProjectSyncButton syncing={syncing} onClick={() => void syncProject()} />
				{syncError ? <p className="mt-2 px-1 text-xs leading-5 text-error">{syncError}</p> : null}
			</footer>
		</div>
	);
}
