...
import {
	...
	listDotnetScripts,
	resolveDotnetLaunch,
	waitForDotnetPortsFree,
} from "../environment/dotnet-runner";
import {
	buildProcessTree,
	getDescendantPids,
	scanListeningPorts,
} from "../environment/environment-scanner";
import { choosePackageManager } from "../environment/scanner";
import { writeProjectExtensionManifest } from "../extensions";
...
export function registerScriptHandlers(ctx: IpcContext) {
	...
	const launchScript = async (
		...
		// Left where an agent will find it: whatever is about to run in this project
		// can read which engines are underlining code before it changes any.
		void writeProjectExtensionManifest(projectPath).catch(() => undefined);
		...
		const runId = ctx.commandRunner.startScript(
			...
			(id, exitCode) =>
				ctx.emitToRenderer("lazify:script-status", {
					runId: id,
					scriptName,
					exitCode,
	...
	ipcMain.handle(
		"lazify:run-script",
		async (_event, projectPath: string, scriptName: string, cols = 220, rows = 50) =>
	...
	ipcMain.handle("lazify:pty-backlog", async (_event, runId: string) =>
		ctx.ptyRunner.getBacklog(runId),
	);
