...
export function registerAgentHandlers(ctx: IpcContext) {
	...
	ipcMain.handle("lazify:agent-usage", async (_event, sinceIso?: string, agentIds?: string[]) =>
		getAgentUsage(sinceIso, agentIds),
	);

	ipcMain.handle("lazify:set-agent-budget", async (_event, agentId: string, weeklyTokens: number) =>
	...
	ipcMain.handle(
		...
		async (
			...
			// Findings from the app's own analysis, wired into this session only —
			// the agent sees them because Lazify opened it, not because anything was
			// written into the project.
			const lint = await prepareSessionLint(agentId, projectPath);

			const runId = ctx.ptyRunner.start(
				definition.binary,
				[...definition.args, ...(resume ?? []), ...lint.args],
				projectPath,
				definition.label,
				cols as number,
				rows as number,
