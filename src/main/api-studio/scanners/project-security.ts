...
export function readProjectSecurity(
  ...
  const applied = named.length > 0 ? named : declaration.guarded ? candidates : [];
...
export function mergeSecurity(
  ...
  return merged.filter((security, at) => merged.findIndex((other) => sameAs(other, security)) === at);
}
