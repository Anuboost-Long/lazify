...
function fromEnvFile(content: string) {
	return content
		.split(/\r?\n/)
		.map((line) => /^\s*(?:BASE_URL|API_URL|APP_URL|PORT|APP_PORT|SERVER_PORT)\s*=(.+)$/.exec(line))
...
function fromEntryFile(content: string) {
	...
	return patterns
		.map((pattern) => pattern.exec(content)?.[1])
...
export async function readProjectServers(
	...
	return Array.from(new Set(found.map((url) => url.trim()).filter((url) => url.startsWith("http"))));
}
