...
const SECRET_NAME = /(token|password|passcode|secret|api[-_]?key|authorization|credential|signature|refresh|otp|pin)/i;
...
export function redactUrl(url: string): string {
  ...
  const query = url
    ...
    .map((pair) => {
      ...
      return SECRET_NAME.test(name) || looksSecret(value)
...
export function redactBody(text: string): string {
  ...
  try {
    ...
    return body
      .replace(
        /("?[\w.-]*(?:token|password|secret|api[-_]?key|authorization|signature)"?\s*[:=]\s*"?)([^"&\s]+)/gi,
