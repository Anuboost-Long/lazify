...
export function readClassName(text: string): string | null {
	return CLASS_PATTERN.exec(text)?.[1] ?? null;
}

export function readBaseTypes(text: string): string[] {
	const inherits =
		/(?:class|record|struct)\s+\w+\s*(?:<[^>]*>\s*)?(?::|extends|implements)([\w<>,. ]+)/.exec(text);
...
export function readMethodDeclaration(lines: string[], start: number): MethodDeclaration | null {
	...
	const match = METHOD_PATTERNS.map((pattern) => pattern.exec(joined)).find(
...
export function readDocSummary(
	...
	const summary = new RegExp(`<${tag}>(.*?)</${tag}>`).exec(collected.join(" "));
