...
function separatorPattern(rules: CallRules) {
	return rules.separator.replace(/[.*+?^${}()|[\]\\]/g, String.raw`\$&`);
}

function callPattern(rules: CallRules, names: string[]) {
	return new RegExp(
		String.raw`(\w+)\s*${separatorPattern(rules)}\s*(${names.join("|")})\s*\(\s*([^,)]+)`,
	);
}
...
function chained(lines: string[], start: number, calls: string[], chainLines: number) {
	...
	return lines
		...
		.some((line) => calls.some((call) => new RegExp(String.raw`\.\s*${call}\s*\(`).test(line)));
}
...
function readSummary(lines: string[], start: number, rules: CallRules) {
	for (let index = start; index < lines.length && index < start + rules.chainLines; index += 1) {
		for (const call of rules.summaryCalls) {
			const match = new RegExp(String.raw`\.\s*${call}\s*\(\s*("[^"]*")`).exec(lines[index]);
...
function readHandlerSignature(lines: string[], start: number, rules: CallRules) {
	...
	if (rules.handlerDeclaration) return rules.handlerDeclaration.exec(text)?.[1] ?? "";
...
function methodsFor(rules: CallRules, name: string, line: string): HttpMethod[] {
	...
	const listed = rules.methodsArgument.pattern.exec(line)?.[1];
	...
	lines.forEach((line, index) => {
		...
		for (const method of methods) {
			routes.push({
				...
				responses: [],
				security: securityFor(rules, lines, index, line.slice(match.index ?? 0), scopes[index].secured),
