...
function groupPattern(rules: CallRules) {
  ...
  const opening = keywords.length > 0 ? `(?:${keywords.join("|")})\\s+` : "";

  return new RegExp(
    `${opening}(\\w+)\\s*=\\s*(?:(\\w+)\\s*\\.\\s*)?(?:${rules.groupCalls.join("|")})\\s*\\(\\s*([^)]*)`
  );
}

function declaredPath(argument: string, rules: CallRules) {
  const named = rules.groups?.pathArgument ? argument.match(rules.groups.pathArgument) : null;
...
export function readMountPrefixes(lines: string[], rules: CallRules, groupNames: Set<string>) {
  ...
  for (const line of lines) {
    for (const mount of patterns) {
      const match = line.match(mount.pattern);
      if (!match) continue;
...
function capturedPrefix(line: string, openers: RegExp[]): string | null {
  for (const opener of openers) {
    const declared = line.match(opener);
