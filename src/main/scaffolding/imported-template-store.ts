...
import {
	createProjectTemplateFromSelection,
	detectFeatures,
	normalizeStructure,
	validateTemplate,
} from "../../brain";
...
function normalizeStoredTemplateSnapshot(
	rawTemplate: Partial<ImportedTemplateSnapshot>,
): ImportedTemplateSnapshot {
	...
	const createdAt =
		rawTemplate.metadata?.createdAt ?? rawTemplate.savedAt ?? new Date().toISOString();
...
function toStoredTemplateSnapshot(
...
export async function listImportedTemplates(): Promise<ImportedTemplateOption[]> {
	...
	return snapshots.map((template) => toOption(template));
}
