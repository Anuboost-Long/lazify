...
import { choosePackageManager, scanEnvironment, type CommandBinary, type PackageManager } from "../environment/scanner";
...
export class WorkflowEngine {
  ...
  async finalizeProject(payload: FinalizeProjectPayload): Promise<WorkflowResult> {
    ...
    if (template.starter) {
      return this.finalizeStarterProject(prepared, workflowId);
    }
    ...
    await this.materializePreparedProject(prepared);
  ...
  private async materializePreparedProject(prepared: PreparedProject): Promise<void> {
  ...
  private async prepareFromStarter(options: {
    ...
    this.emitProgress({ workflowId, status: "running", step: "starter-ready", message });
