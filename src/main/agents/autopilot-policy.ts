...
const CRITICAL_GUARDS: Guard[] = [
  ...
  { name: "git-history-rewrite", pattern: /\bgit\s+(?:rebase|filter-branch|filter-repo|reflog\s+delete)\b/i },
  { name: "git-branch-delete", pattern: /\bgit\s+branch\s+(?:-\w+\s+)*-D\b|\bgit\s+push[^\n]*--delete\b/i },
  ...
  { name: "deploy", pattern: /\b(?:vercel|netlify|firebase|flyctl|fly|heroku|eas|expo|wrangler|serverless|sls|gcloud|aws|az)\s+(?:\w+\s+)*(?:deploy|publish|submit|release|push)\b/i },
  ...
  { name: "infra-apply", pattern: /\b(?:terraform|pulumi)\s+(?:apply|destroy)\b|\bkubectl\s+(?:apply|delete|drain)\b/i },
  { name: "outbound-post", pattern: /\bcurl\b[^\n]*(?:-X\s*(?:POST|PUT|DELETE|PATCH)|--data|-d\s)/i },
  ...
  // ── Databases ──────────────────────────────────────────────────────────
  { name: "destructive-sql", pattern: /\b(?:drop\s+(?:table|database|schema)|truncate\s+table|delete\s+from)\b/i },
  { name: "migration-reset", pattern: /\b(?:prisma|drizzle-kit|sequelize|knex|alembic|rails)\b[^\n]*\b(?:reset|drop|db:drop|migrate:reset|downgrade)\b|\bdb\s+push\b[^\n]*--(?:force|accept-data-loss)/i },

  // ── Secrets and anything outside the project ───────────────────────────
  // Reading a secret is as bad as writing one: it lands in the transcript.
  { name: "credential-file", pattern: /(?:^|[\s/'"])(?:\.env(?:\.\w+)?|\.npmrc|\.netrc|id_[re]d?sa|[\w-]+\.pem|[\w-]+\.p12|credentials|secrets?\.(?:ya?ml|json|ts|js))\b/i },
  { name: "credential-dir", pattern: /~?\/\.(?:ssh|aws|gnupg|kube|docker|config\/gh|gcloud)\b/i },
  { name: "keychain", pattern: /\b(?:security\s+(?:add|find|delete)-\w*password|keychain|op\s+read|vault\s+(?:read|write))\b/i },
  { name: "secret-env", pattern: /\b(?:AWS_SECRET|API_KEY|ACCESS_TOKEN|PRIVATE_KEY|CLIENT_SECRET|PASSWORD)\s*=\s*\S/i },
  { name: "home-wide", pattern: /(?:^|\s)(?:~|\$HOME)\/?(?:\s|$)|(?:^|\s)rm\b[^\n]*\s(?:~|\$HOME)\b/i },
  ...
  { name: "bypass-permissions", pattern: /\bbypass permissions\b|\bdangerously[- ]skip\b|\byolo mode\b|\bskip all permission\b/i },
  { name: "mcp-trust", pattern: /\bmcp server\b|\bnew mcp\b/i },
  { name: "login", pattern: /\b(?:log ?in|sign ?in|authenticate|authorize)\b[^\n]{0,40}\?|\bapi key\b|\boauth\b/i }
];
...
const OPINION_OPTION_GUARDS: Guard[] = [
  { name: "plan-mode", pattern: /\bkeep planning\b|\bauto[- ]?accept edits\b|\bmanually approve edits\b|\brevise the plan\b/i },
  { name: "pick-alternative", pattern: /\b(?:instead|either way|another approach|different approach)\b/i }
];
...
const OPINION_GUARDS: Guard[] = [
  { name: "preference", pattern: /\b(?:which|what) (?:would you|do you|one|option|approach|of these)\b/i },
  { name: "prefer", pattern: /\b(?:prefer|preference|your call|up to you|thoughts\?)\b/i },
  { name: "should-i", pattern: /\b(?:should|shall) (?:i|we)\b/i },
  { name: "also-instead", pattern: /\b(?:want me to|would you like me to)\b[^\n]*\b(?:also|instead|too|as well)\b/i },
  { name: "naming", pattern: /\bwhat (?:should|shall) (?:i|we|it) (?:call|name)\b|\bname (?:it|this|the)\b[^\n]*\?/i },
  { name: "plan-approval", pattern: /\b(?:plan|approach|design|strategy|proposal)\b[^\n]{0,40}\?/i },
  { name: "keep-going", pattern: /\b(?:continue|proceed) (?:with|to) (?:the )?(?:next|rest|remaining)\b/i }
];

/** A plain, one-time "go ahead". */
const AFFIRMATIVE = /^(?:yes|y|allow|approve|proceed|continue|ok(?:ay)?|confirm|accept|apply|do it|go ahead)\b/i;
 ...
 */
const WIDENING = /\b(?:don'?t ask again|do not ask again|always|all (?:edits|commands|future)|rest of (?:this )?session|during this session|for (?:this|the) session|yes to all|auto[- ]?accept)\b/i;
