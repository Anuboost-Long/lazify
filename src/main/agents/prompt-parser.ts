...
const YES_NO = /(?:^|\s)(.{0,120}?\?)?\s*[[(](?:y\/n|yes\/no|Y\/n|y\/N)[\])]\s*:?\s*$/im;

/** A field to type into: "Enter your API key:", "Paste the token:". */
const INPUT_REQUEST = /^\s*(?:please\s+)?(?:enter|type|paste|provide|supply)\b([^\n]{0,80}):\s*$/im;
...
/** Footer hints and separators — chrome, never content. */
const CHROME =
  /^(?:[─━═╌─-╿\s]*|esc(?:ape)? to .*|(?:shift\+)?tab to .*|enter to .*|\? for shortcuts.*|↑\/↓.*|arrow keys? to .*)$/i;
...
function readOptions(lines: string[]): { options: PromptOption[]; from: number } | null {
  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    if (OPTION_LINE.test(lines[index])) {
      last = index;
      break;
    }
  }
  ...
  for (; index >= 0 && found.length < 12; index -= 1) {
    ...
    if (!match) {
      if (!lines[index].trim() || found.length === 0) continue;
...
export function parseAgentPrompt(screen: string): AgentPrompt | null {
  ...
  const tail = lines.filter((line) => line).slice(-MAX_BOX_LINES);
