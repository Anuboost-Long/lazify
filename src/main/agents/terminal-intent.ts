...
const QUESTION_SIGNALS: Signal[] = [
	...
	{
		name: "caret-on-option",
		pattern: /(?:^|\n)[ \t]*[>❯▶→›»▸]\s*\d+[.)]\s+\S/,
		weight: 3,
		blocking: true,
	},
	{ name: "do-you-want-to", pattern: /\bDo you want to\b/i, weight: 3, blocking: true },
	{ name: "yes-no-bracket", pattern: /\??\s*[[(](?:y\/n|yes\/no)[\])]/i, weight: 3, blocking: true },
	{
		name: "waiting-for-you",
		pattern: /\bwaiting for (?:your|user|a) (?:input|response|reply|answer|approval|confirmation)\b/i,
		weight: 3,
		blocking: true,
	},
	...
	{
		name: "permission-wording",
		pattern: /\bpermission (?:required|request(?:ed)?|denied|to (?:run|use|edit|read|write))\b/i,
		weight: 2,
		blocking: true,
	},
	{
		name: "approve-question",
		pattern: /\b(?:approve|allow|grant|authorize)\b[^\n?]{0,80}\?/i,
		weight: 2,
		blocking: true,
	},
	{
		name: "enter-to-confirm",
		pattern: /\bEnter to (?:select|confirm|submit|choose|continue|accept)\b/i,
		weight: 2,
		blocking: true,
	},
	...
	// when you get a chance") do not read as a prompt.
	{
		name: "input-request",
		pattern:
			/(?:^|\n)[ \t]*(?:please\s+)?(?:enter|type|paste|provide|supply)\b[^\n:]{0,60}:[ \t]*$/im,
		weight: 3,
		blocking: true,
	},
	...
	// cycle") is the idle footer's mode switch, which is the opposite of a menu.
	{
		...
		pattern:
			/(?<![\w+-])(?:tab|arrow keys?|↑\/↓|up\/down) to (?:navigate|move|cycle|switch|select)\b/i,
		weight: 2,
		blocking: true,
	},
	{
		name: "escape-to-cancel",
		pattern: /\bEsc(?:ape)? to (?:cancel|exit|go back|reject)\b/i,
		weight: 1,
		blocking: true,
	},
	...
	// to answer, change the subject, or walk away.
	{
		...
		pattern: /\b(?:would you like|should i|shall i|may i|can i|want me to)\b[^\n]{0,120}\?/i,
		weight: 2,
	},
	{
		name: "select-an-option",
		pattern: /\b(?:select|choose|pick)\s+(?:an?|one|which|from|the)\b/i,
		weight: 2,
	},
...
const COMPLETE_SIGNALS: Signal[] = [
	{
		name: "idle-footer-hint",
		pattern: /(?:shift\+tab to cycle|\? for shortcuts|for shortcuts\b)/i,
		weight: 3,
		authoritative: true,
	},
	{
		name: "idle-footer-mode",
		pattern: /\b(?:bypass permissions|accept edits|plan mode|auto-accept edits)\s+(?:on|off)\b/i,
		weight: 3,
		authoritative: true,
	},
	...
	{
		name: "completion-word",
		pattern: /\b(?:done|completed|finished|all set|ready)\b[.!]?\s*$/im,
		weight: 2,
	},
	{
		name: "clean-result",
		pattern: /\b(?:tests? passed|build succeeded|no changes|nothing to commit|up to date)\b/i,
		weight: 2,
	},
...
const WORKING_SIGNALS: Signal[] = [
	{ name: "interrupt-hint", pattern: /\besc(?:ape)? to interrupt\b/i, weight: 3 },
	{
		name: "status-verb",
		pattern:
			/\b(?:thinking|working|running|searching|reading|editing|writing|waiting|analyzing|building|testing|fetching|planning|processing|generating|compiling|installing)\s*[.…]{1,3}/i,
		weight: 3,
	},
	// The live counter an agent prints beside its spinner: "(12s · ↑ 1.2k tokens)".
	{ name: "token-counter", pattern: /\(\s*\d+(?:m\d*)?s\s*[·•]/, weight: 2 },
	{
		name: "in-progress-tool",
		pattern: /\b(?:running|executing|calling)\s+(?:command|tool|script)\b/i,
		weight: 2,
	},
];
...
export function stripAnsi(value: string): string {
	...
	return (
		value
			...
			.replace(/\u001b\][\s\S]*?(?:\u0007|\u001b\\)/g, "") // NOSONAR: ESC and BEL are the sequence being stripped
			...
			.replace(/\u001b[[\]]*[0-9;?]*[ -/]*[A-Za-z]/g, "") // NOSONAR: ESC is the sequence being stripped
 ...
 */
/** Why a match counted for less than its full weight. */
function discountOf(live: boolean, code: boolean): string {
	if (code) return "(code)";
...
/**
 * The strongest occurrence of one signal, and how far down the screen it reached.
 *
 * Every occurrence is weighed and the strongest one speaks for the signal: a
 * phrase quoted in a diff higher up must not drown out the same phrase being
 * asked for real at the bottom, nor the other way round.
 */
function scanSignal(pattern: RegExp, text: string, liveFrom: number) {
	...
	for (let match = scanner.exec(text); match; match = scanner.exec(text)) {
		...
		if (factor > best) {
			best = factor;
			discounted = discountOf(live, code);
		}
...
function scoreSignals(signals: Signal[], text: string, liveFrom: number): ClassScore {
	...
	for (const signal of signals) {
		const { end, best, discounted } = scanSignal(signal.pattern, text, liveFrom);
