...
/** Why the build cannot start, or null when it can. */
function refusal(appPath: string, outputPath: string): string | null {
	const problem = bundleProblem(appPath);
	if (problem) return problem;

	if (!outputPath.toLowerCase().endsWith(".dmg")) {
		return "The output file needs a .dmg extension.";
	}
	...
	if (path.resolve(outputPath).startsWith(`${path.resolve(appPath)}${path.sep}`)) {
		return "Choose a destination outside the app itself.";
	}

	return null;
}

/** Runs a step the build cannot go on without; its own words carry the failure. */
async function step(command: string, args: string[], fallback: string, timeoutMs?: number) {
...
/**
 * The backdrop, staged rather than written after mounting, because Finder has to
 * be able to point at it in the same breath as it sets the layout. Dot-prefixed
 * so the window shows two icons and nothing else.
 */
async function stageBackground(stage: string, imagePath: string | null | undefined) {
	const nothing = { fileName: null, size: null, warning: null };

	if (!imagePath || !fs.existsSync(imagePath)) return nothing;
	...
	if (!(await writePng(imagePath, background))) {
		return {
			...nothing,
	...
	return { fileName: "background.png", size: await imagePixelSize(background), warning: null };
}

/**
 * After the layout, never before: Finder deletes `.VolumeIcon.icns` and clears
 * the custom-icon flag as it writes the window out.
 */
async function applyVolumeIcon(
	appPath: string,
	mountPoint: string,
	chosen: string | null | undefined,
) {
	const source =
		chosen && fs.existsSync(chosen) ? chosen : findBundleIcon(appPath, await readInfoPlist(appPath));

	if (source && (await writeIcns(source, path.join(mountPoint, ".VolumeIcon.icns")))) {
...
export async function compileDmg(
	...
	const refused = refusal(appPath, outputPath);
	...
	try {
		...
		await step(
			"ditto",
			[appPath, path.join(stage, appFileName)],
			"Copying the app into the image failed.",
		);

		// The drag-to-install half of the window.
		fs.symlinkSync("/Applications", path.join(stage, "Applications"));

		const backdrop = await stageBackground(stage, options.backgroundImagePath);

		warning = backdrop.warning;

		const layout = windowLayout(backdrop.size);
		...
		await step(
			...
			"Building the disk image failed.",
		);

		// `-nobrowse` and `-noautoopen` keep the half-built image from flashing up
		// in the user's Finder while this works on it.
		const attach = await step(
			"hdiutil",
			["attach", scratchImage, "-nobrowse", "-noverify", "-noautoopen"],
			"The disk image could not be mounted to style it.",
		);
		...
		fs.writeFileSync(script, layoutScript(diskName, appFileName, layout, backdrop.fileName));
		...
		await applyVolumeIcon(appPath, mountPoint, options.volumeIconPath);
		...
		await step(
			...
			"Compressing the disk image failed.",
		);
