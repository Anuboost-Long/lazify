...
const SCRIPTS: Array<{ name: string; args: (target: string | null, project: string | null) => string[] }> = [
  ...
  { name: "publish", args: (_target, project) => (project ? ["publish", project, "-c", "Release"] : ["publish", "-c", "Release"]) },
  ...
  try {
    ...
    for (const profile of Object.values(parsed.profiles ?? {})) {
      for (const url of (profile.applicationUrl ?? "").split(";")) {
        const port = Number(url.trim().match(/:(\d{2,5})(?:\/|$)/)?.[1]);
