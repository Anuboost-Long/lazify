...
function definitionAt(projectPath: string, filePath: string, line: number, kind: SymbolDefinition["kind"]): SymbolDefinition {
  return {
 ...
 */
type Trail =
  | { kind: "found"; filePath: string; source: string }
  | { kind: "outside" }
  | { kind: "unknown" };
...
async function fileDeclaring(
  ...
  for (let hop = 0; hop <= MAX_RE_EXPORT_HOPS; hop += 1) {
    ...
    const specifier =
      hop === 0
        ? importSpecifierOf(currentSource, name)
        : reExportSpecifierOf(currentSource, name);
...
function propsTypeOf(source: string, component: string): string | null {
  ...
  const patterns = [
    ...
    new RegExp(`function\\s+${name}\\s*\\([\\s\\S]*?:\\s*(?:Readonly<)?([A-Za-z_$][\\w$]*)`),
    // const Button = ({ a }: ButtonProps) => / const Button: FC<ButtonProps> =
    new RegExp(`const\\s+${name}\\s*(?::[^=]*?<\\s*([A-Za-z_$][\\w$]*)|=[\\s\\S]{0,200}?:\\s*(?:Readonly<)?([A-Za-z_$][\\w$]*))`)
  ];
...
function memberLineIn(lines: string[], from: number, prop: string): number | null {
  const member = new RegExp(`^\\s*(?:readonly\\s+)?["']?${prop.replace(/[$]/g, "\\$&")}["']?\\??\\s*[:(<]`);
  let depth = 0;
...
function propLineIn(source: string, component: string, prop: string): number | null {
  ...
  if (typeName) {
    const declaration = new RegExp(
      `^\\s*(?:export\\s+)?(?:interface|type)\\s+${typeName.replace(/[$]/g, "\\$&")}\\b`
    );
...
export async function findReferenceDefinition(
  ...
  if (fromPath && source && position) {
    ...
    if (attribute && attribute.prop === symbol) {
