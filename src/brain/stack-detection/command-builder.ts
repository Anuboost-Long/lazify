 ...
 */
export function buildDotnetCommands(
...
export function buildCommands(input: {
	...
	switch (input.stack) {
		...
		case "react-next":
			commands.dev =
				getScriptCommand(input.packageManager, scripts, "dev") ??
				`${runnerCommand(input.packageManager)} next dev`;
			commands.build =
				getScriptCommand(input.packageManager, scripts, "build") ??
				`${runnerCommand(input.packageManager)} next build`;
			commands.start =
				getScriptCommand(input.packageManager, scripts, "start") ??
				`${runnerCommand(input.packageManager)} next start`;
			assignScriptIfPresent("test", "test");
			assignScriptIfPresent("lint", "lint");
			break;
		case "react-vite":
			commands.dev =
				getScriptCommand(input.packageManager, scripts, "dev") ??
				`${runnerCommand(input.packageManager)} vite`;
			commands.build =
				getScriptCommand(input.packageManager, scripts, "build") ??
				`${runnerCommand(input.packageManager)} vite build`;
			commands.preview =
				getScriptCommand(input.packageManager, scripts, "preview") ??
				`${runnerCommand(input.packageManager)} vite preview`;
			assignScriptIfPresent("test", "test");
			assignScriptIfPresent("lint", "lint");
			break;
		case "react-cra":
		...
		case "electron":
			...
			assignScriptIfPresent("build", "build") ||
				assignScriptIfPresent("build", "dist") ||
				assignScriptIfPresent("build", "make") ||
				assignScriptIfPresent("build", "package");
			assignScriptIfPresent("test", "test");
			assignScriptIfPresent("lint", "lint");
			break;
		// Nothing framework-specific to add: the plain script names are the whole
		// vocabulary these stacks have.
		case "node-api":
		case "react-unknown":
		default:
			assignScriptIfPresent("dev", "dev");
