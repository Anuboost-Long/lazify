...
const SWIFTUI_MARKERS: Array<{ pattern: RegExp; reason: string }> = [
  { pattern: /^\s*import\s+SwiftUI\b/m, reason: "imports SwiftUI" },
...
const UIKIT_MARKERS: Array<{ pattern: RegExp; reason: string }> = [
  { pattern: /^\s*import\s+UIKit\b/m, reason: "imports UIKit" },
...
function packageNameOf(contents: string): string | null {
  return contents.match(/name:\s*"([^"]+)"/)?.[1] ?? null;
}
...
export async function detectSwiftProject(projectRoot: string): Promise<SwiftDetectionResult> {
  ...
  result.scheme = schemeSource
    ? path.basename(schemeSource, path.extname(schemeSource))
    : result.packageFile
      ? packageNameOf(await readHead(projectRoot, result.packageFile))
      : null;
