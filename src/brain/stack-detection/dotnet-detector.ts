  ...
  const scan = async (relativeDirectory: string, depth: number) => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (depth + 1 < MAX_SCAN_DEPTH && !SKIPPED_DIRECTORIES.has(entry.name) && !entry.name.startsWith(".")) {
      ...
      if (SOLUTION_EXTENSIONS.includes(extension)) solutions.push(relativePath);
      if (PROJECT_EXTENSIONS.includes(extension)) projects.push(relativePath);
    }
  };
...
function targetFrameworkOf(contents: string): string | null {
  const single = contents.match(/<TargetFramework>\s*([^<\s]+)\s*<\/TargetFramework>/i);
  if (single) return single[1];

  const multiple = contents.match(/<TargetFrameworks>\s*([^<]+)\s*<\/TargetFrameworks>/i);
